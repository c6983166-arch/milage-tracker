...
public final class YearEndReport {
    public static File create(Context c,int year,List<DatabaseHelper.Trip> trips)throws Exception{
        ...
        for(int idx=-1;idx<trips.size();idx++){
            ...
            if(idx==-1){ p.setTextSize(12);p.setTypeface(Typeface.DEFAULT_BOLD);canvas.drawText(String.format(Locale.US,"Estate Miles: %.1f",estate),36,y,p);canvas.drawText(String.format(Locale.US,"Business Miles: %.1f",business),210,y,p);canvas.drawText(String.format(Locale.US,"Total: %.1f",estate+business),390,y,p);y+=28; p.setTextSize(9);p.setTypeface(Typeface.DEFAULT_BOLD);canvas.drawText("DATE",36,y,p);canvas.drawText("TYPE / ESTATE",100,y,p);canvas.drawText("PURPOSE",285,y,p);canvas.drawText("MILES",540,y,p);y+=14;canvas.drawLine(36,y,576,y,p);y+=14; continue; }
            DatabaseHelper.Trip t=trips.get(idx); p.setTextSize(8.5f);p.setTypeface(Typeface.DEFAULT);p.setColor(Color.DKGRAY); canvas.drawText(s(t.date,16),36,y,p);String type="ESTATE".equals(t.tripType)?"Estate • "+t.estateName:"Business";canvas.drawText(s(type,32),100,y,p);canvas.drawText(s(t.purpose,40),285,y,p);canvas.drawText(String.format(Locale.US,"%.1f",t.miles),540,y,p);y+=16;
        }
